...
showMove (Move Castle _ from to)
    | from - to == 2 = "0-0-0"
    | otherwise = "0-0"
showMove (Move Promote piece from to) =
    show (bitToCoordinate from) ++ show (bitToCoordinate to) ++ "=" ++ show piece
showMove (Move EnPassant piece from to) =
    show piece ++ show (bitToCoordinate from) ++ show (bitToCoordinate to) ++ "E.P."
showMove (Move Relocate piece from to) =
    show piece ++ show (bitToCoordinate from) ++ show (bitToCoordinate to)